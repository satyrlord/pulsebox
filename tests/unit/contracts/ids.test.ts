...
describe("opaque identifiers", () => {
  ...
  it("accepts only lowercase canonical UUID version 4 values", () => {
    ...
    expect(parseCanonicalUuid("00000000-0000-1000-8000-000000000001").ok).toBe(false);
  });
  ...
  it("uses an injected deterministic UUID factory", () => {
    ...
    expect(() => createCanonicalUuid({ createUuid: () => "not-a-uuid" })).toThrow(/non-canonical/u);
  });
  ...
  it("validates rack, content, and project revision bounds", () => {
    ...
    expect(validateProjectRevision({ epoch: TEST_UUID, counter: 0 }).ok).toBe(true);
    expect(validateProjectRevision({ epoch: TEST_UUID, counter: Number.MAX_SAFE_INTEGER }).ok).toBe(
      true,
    );
    expect(validateProjectRevision({ epoch: TEST_UUID, counter: -1 }).ok).toBe(false);
