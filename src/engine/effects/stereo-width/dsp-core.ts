...
export class StereoWidthDsp implements EffectFrameProcessor {
  ...
  process(left: number, right: number, output?: StereoFrame): StereoFrame { const l = finite(left), r = finite(right), mid = (l + r) * 0.5, side = (l - r) * 0.5; const hp = numberState(this.#state, "high-pass", 120, 20, 2000), lp = numberState(this.#state, "low-pass", 14000, 2000, 20000); const filteredSide = this.#lowpass.process(this.#highpass.process(side, hp, this.#sampleRate), lp, this.#sampleRate); const width = numberState(this.#state, "width", 1, 0, 2), mix = numberState(this.#state, "mix", 1, 0, 1); const wetLeft = mid + filteredSide * width, wetRight = mid - filteredSide * width; return writeStereoFrame(output, finite(equalPowerMix(l, wetLeft, mix)), finite(equalPowerMix(r, wetRight, mix))); }
}
