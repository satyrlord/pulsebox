...
class Allpass { #state = 0; reset(): void { this.#state = 0; } process(input: number, coefficient: number): number { const output = -coefficient * input + this.#state; this.#state = input + coefficient * output; return output; } }
export class PhaserDsp implements EffectFrameProcessor {
  readonly #sampleRate: number; readonly #state: EffectState; readonly #left = [new Allpass(), new Allpass(), new Allpass(), new Allpass()]; readonly #right = [new Allpass(), new Allpass(), new Allpass(), new Allpass()]; #phase = 0; #feedbackLeft = 0; #feedbackRight = 0;
  constructor(sampleRate: number, state: EffectState) { this.#sampleRate = sampleRate; this.#state = state; }
  reset(): void { for (const stage of [...this.#left, ...this.#right]) stage.reset(); this.#phase = 0; this.#feedbackLeft = 0; this.#feedbackRight = 0; }
