/**
 * Boom Eight: eight independent synthesized drum voices with a compressed
 * mix bus.
 *
 * The large analog-style machine trades Drumline Six's Snap-and-Drive
...
class BoomVoice {
  ...
  setParameters(parameters: BoomVoiceParameters): void {
    this.#parameters = parameters;
  }
...
export class BoomEightDsp {
  ...
  setParameters(
    ...
    for (const id of BOOM_VOICE_IDS) this.#voices.get(id)?.setParameters(voices[id]);
  }
