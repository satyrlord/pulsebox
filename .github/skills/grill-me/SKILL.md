---
name: grill-me
description: Stress-test a Pulsebox plan or design interactively, resolve consequential decision branches, and expose hidden assumptions before implementation.
---
...
- Probe scope, engine/state/UI ownership, plugin and worklet contracts,
  ...
  support, responsive layout, originality, verification, and delivery risk
  when relevant.

Update the owning specification or domain document after each accepted product
decision. Until the planned rename occurs, use
docs/pulsebox-unified-spec-v1.0.md as the single product specification. Do not
create a parallel decision document.
...
Finish when every inventoried branch is resolved or explicitly deferred with
an owner, reason, and verification step, and a fresh agent can continue without
reopening decisions.
