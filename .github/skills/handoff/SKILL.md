...
name: handoff
description: >
  Create a Pulsebox session or phase handoff. Use for work that a fresh agent
  must resume, close, or verify without relying on conversation history.
---
...
1. Use the reply or an ignored temporary path for the handoff.
   Completion criterion: No handoff file will enter the repository tree.
2. Link each recorded durable fact to its owning document.
   Completion criterion: The temporary narrative owns no product contract.
3. Link to current owners instead of copying their full contents.
   Completion criterion: No fact has a second owner in the handoff.
...
1. Inspect HEAD, status, the complete diff, and the last concrete action.
   Completion criterion: The handoff distinguishes current work from unrelated dirty work.
2. Inspect the latest relevant check results.
   Completion criterion: Each reported pass or failure has current command evidence.
3. List the least-confident work with a verification procedure.
   Completion criterion: Every uncertainty has one concrete check.
4. List skipped, incomplete, and deferred work.
   Completion criterion: No completion claim hides a known gap.
5. State previously unstated assumptions and the largest remaining blind spot.
   Completion criterion: The next agent can test each material assumption.
...
The handoff is complete after a fresh agent can identify the task, verified
state, owners, blockers, and exact next action. For finished work, the agent
must be able to verify that no action remains.
