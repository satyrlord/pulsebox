...
1. Read `AGENTS.md`, the specification index, each applicable owner, and each dependency.
   Finish when you know the required phase and acceptance checks.
2. Read `package.json` and each active tool configuration.
   Finish when you know every configured command and project.
3. Use `package.json` as the only source for repository commands.
   Finish when every planned command comes from current package scripts.
...
Complete repair mode after you fix and recheck each in-scope root cause. State
